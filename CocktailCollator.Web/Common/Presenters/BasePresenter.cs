...
{
    protected readonly ToastService ToastService = toastService;

    public virtual Task Unauthenticated(CancellationToken cancellationToken)
    {
        this.ToastService.ShowToast(ToastType.Danger, "Access Denied", $"You must be logged in to {actionDescription}.");
        return Task.CompletedTask;
    }

    public virtual Task Unauthorised(CancellationToken cancellationToken)
    {
        this.ToastService.ShowToast(ToastType.Danger, "Permission Denied", $"You do not have permission to {actionDescription}.");
        return Task.CompletedTask;
    }

    public virtual Task NotFound(CancellationToken cancellationToken)
    {
        this.ToastService.ShowToast(ToastType.Warning, "Not Found", $"The item you are trying to {actionDescription} for does not exist.");
        return Task.CompletedTask;
    }
}